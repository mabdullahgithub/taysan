...
class OrderController extends BaseController
{
    ...
    public function storeWebOrders(Request $request)
    {
        try {
            ...
            $tolerance = 50; // Increased tolerance to 50 PKR to handle price mismatches while we debug
            if (abs($finalTotal - $validated['total']) > $tolerance) {
                Log::warning('Large price mismatch detected', [
                ...
                // For large discrepancies, use calculated total but continue with order
                Log::info('Using calculated total due to large price mismatch', [
                ...
                // For smaller discrepancies, log but continue
                Log::info('Small price mismatch detected, using calculated total', [
                    'calculated_total' => $finalTotal,
                    'submitted_total' => $validated['total'],
                    'difference' => abs($finalTotal - $validated['total']),
                    'using_calculated' => $finalTotal
